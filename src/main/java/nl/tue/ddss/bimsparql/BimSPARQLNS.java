package nl.tue.ddss.bimsparql;

public class BimSPARQLNS {
	
	public final static String SPT="http://bimsparql.org/spatial#";
	public final static String PDT="http://bimsparql.org/product#";
	public final static String PSET="http://bimsparql.org/pset#";
	public final static String QTO="http://bimsparql.org/qset#";
	public final static String SCHM="http://bimsparql.org/schema#";
	public final static String GEOM="http://bimsparql.org/geometry#";

}
