...
public class IntersectsPF extends GeometryFunctionBase{
    ...
    protected List<Node> getRelatedNodes(Node node,ExecutionContext execCxt){
		...
		for (Node n:hashmap.keySet()){
			try {
				if(Intersects.intersects3D(geometry, getGeometry(n,execCxt.getActiveGraph()))==1){
					nodes.add(n);
				}
	...
	protected QueryIterator verify(Binding binding, Graph queryGraph,
		...
		try {
			if(Intersects.intersects3D(s, o)==1){
