...
public class DisjointsPF extends GeometryFunctionBase{
	
	public DisjointsPF(HashMap<Node, Geometry> hashmap) {
		super(hashmap);
	}
    protected List<Node> getRelatedNodes(Node node,ExecutionContext execCxt){
		...
		for (Node n:hashmap.keySet()){
			try {
				if(Intersects.intersects3D(geometry, getGeometry(n,execCxt.getActiveGraph()))==0){
					nodes.add(n);
				}
	...
	protected QueryIterator verify(Binding binding, Graph queryGraph,
		...
		try {
			if(Intersects.intersects3D(s, o)==0){
