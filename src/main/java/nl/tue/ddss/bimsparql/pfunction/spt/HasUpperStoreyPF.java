...
public class HasUpperStoreyPF extends FunctionBaseSpatialRelationOnGraph {

	public HasUpperStoreyPF(HashMap<Node, Geometry> hashmap) {
		super(hashmap);
	...
	private double elevation(Node storey, Graph graph) {
		double elevation;
		ExtendedIterator<Triple> it=graph.find(storey,NodeFactory.createURI(Namespace.IFC2X3_TC1+"elevation_IfcBuildingStorey"),null);
		while(it.hasNext()){
			Node obj=it.next().getObject();
			ExtendedIterator<Triple> it2=graph.find(obj,NodeFactory.createURI(Namespace.EXPRESS+"hasDouble"),null);
			Node result=it2.next().getObject();
			return (Double)result.getLiteralValue();
		}
		AABB aabb = new AABB();
		ExtendedIterator<Triple> iterator2 = graph.find(null,
				NodeFactory.createURI(Namespace.IFC2X3_TC1 + "relatingStructure_IfcRelContainedInSpatialStructure"),
				storey);
		while (iterator2.hasNext()) {
			Node rel = iterator2.next().getSubject();
			ExtendedIterator<Triple> iter = graph.find(rel,
					NodeFactory.createURI(Namespace.IFC2X3_TC1 + "relatedElements_IfcRelContainedInSpatialStructure"),
					null);
			while (iter.hasNext()) {
...
//				}
		...
		if (aabb.getMin() != null && aabb.getMax() != null) {
