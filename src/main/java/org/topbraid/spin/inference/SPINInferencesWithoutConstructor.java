 ...
 */
public class SPINInferencesWithoutConstructor { 
	 ...
	 * @return the number of iterations (1 with singlePass)
	 */
	public static int runWithoutConstructors(
			Model queryModel,
			Model newTriples,
			Map<Resource, List<CommandWrapper>> class2Query,
			SPINExplanations explanations,
		...
		do {
			...
			for(CommandWrapper arqWrapper : rulesList) {
				...
				String explanationText = sb.toString();
				boolean thisUnbound = arqWrapper.isThisUnbound();
				changed |= runCommandOnClass(arqWrapper, arqWrapper.getLabel(), queryModel, newTriples, cls, true, statistics, explanations, explanationText, newRules, thisUnbound, monitor);
				if(!SPINUtil.isRootClass(cls) && !thisUnbound) {
					Set<Resource> subClasses = JenaUtil.getAllSubClasses(cls);
					for(Resource subClass : subClasses) {
						changed |= runCommandOnClass(arqWrapper, arqWrapper.getLabel(), queryModel, newTriples, subClass, true, statistics, explanations, explanationText, newRules, thisUnbound, monitor);
					}
				}
			}
			iteration++;
	...
	private static boolean runCommandOnClass(
		...
		if(thisUnbound || SPINUtil.isRootClass(cls) || queryModel.contains(null, RDF.type, cls)) {
			...
			if(!newInstances.isEmpty()&&!class2Constructor.isEmpty()) {
